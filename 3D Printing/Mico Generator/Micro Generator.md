...
| **Magnet Dimensions** | Individual magnet size | $7.1 \text{ mm } \text{(L)} \text{ x } 2.7 \text{ mm } \text{(W)} \text{ x } 2 \text{ mm } \text{(H)}$ | Constraint |
| **Wire Gauge** | Copper conductor | AWG 20 (Bare Diameter: $\approx 0.812 \text{ mm}$) | Constraint |
| **Air Gap** | Spacing between rotor magnets and stator cores | $\mathbf{\leq 1.5 \text{ mm}}$ | Critical for maximizing voltage output [2] |
...
| **Magnet Pockets** | $7.1 \text{ x } 2.7 \text{ x } 2 \text{ mm}$ (plus tolerance) | The disk must be printed with recessed pockets to hold each magnet securely, preventing detachment under centrifugal force. |
